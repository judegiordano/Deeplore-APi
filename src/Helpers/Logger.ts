...
export class Log {
	...
	public static Info(data: any, service = "user service"): void {
	...
	public static Warn(data: any, service = "user service"): void {
		const time = Log.HumanDate().toString();
		const log = `{ service: "${service}", level: "warn", timestamp: ${new Date().getTime()}, date: "${time}", message: "${data.toString()}" }\n`;
		process.stdout.write(`\x1b[33m${log}\x1b[0m`);
		fs.appendFileSync(Log.warnPath, log);
	}

	public static Error(data: any, service = "user service"): void {
		const time = Log.HumanDate().toString();
		const log = `{ service: "${service}", level: "error", timestamp: ${new Date().getTime()}, date: "${time}", message: "${data.toString()}" }\n`;
		process.stdout.write(`\x1b[31m${log}\x1b[0m`);
